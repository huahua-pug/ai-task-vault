...
[EN/中] Appendix B (proof of Theorem 4.1): using the jump identity ⟨n×a, b⟩_{∂T_h} = ⟨⟦a⟧_K, {‖b‖}⟩_{F⁰_h} − ⟨{‖a‖}, ⟦b⟧_K⟩_{F⁰_h} + ⟨n×a, b⟩_Γ (Lemma B.1, proven in Appendix D), the proof shows the deviation terms θ¹_{E_h} = ⟨C11⟦E_h⟧_K, ⟦v⟧_K⟩ and θ_{H_h} = ⟨C22⟦H_h⟧_K, ⟦r⟧_K⟩ vanish if and only if C11 = C22 = 0 — hence DG is Hamiltonian iff C11 = C22 = 0, and HDG never (its own hybrid equation introduces an unavoidable extra term).｜附录B（定理4.1证明）：借助跳跃恒等式（引理B.1，附录D证明），把偏差量归为 ⟨C11⟦E_h⟧, ⟦v⟧⟩ 与 ⟨C22⟦H_h⟧, ⟦r⟧⟩ 两项，当且仅当 C11 = C22 = 0 时恒为零——DG 的哈密顿性条件由此而来；HDG 因杂交方程引入不可消除的额外项而永不满足。
